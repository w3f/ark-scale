...
where W: Write+?Sized, C: sw::SWCurveConfig,
{
...
where O: Output+?Sized, C: sw::SWCurveConfig,
{
...
where R: Read, C: sw::SWCurveConfig,
{
...
where I: Input, C: sw::SWCurveConfig,
{
...
where W: Write+?Sized, C: te::TECurveConfig,
{
    p.x.serialize_with_mode(&mut *dest, MC) ?;
    p.y.serialize_with_mode(&mut *dest, MC) ?;
...
where O: Output+?Sized, C: te::TECurveConfig,
{
...
where R: Read, C: te::TECurveConfig,
{
    let x = C::BaseField::deserialize_with_mode(&mut *src, MC, MV) ?;
    let y = C::BaseField::deserialize_with_mode(&mut *src, MC, MV) ?;
...
where I: Input, C: te::TECurveConfig,
{
